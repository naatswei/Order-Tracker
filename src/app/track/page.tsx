...
export default function TrackPage() {
    ...
    return (
        <div className="min-h-screen bg-[#0A0B14] text-white selection:bg-[#85A39E]/30 overflow-hidden relative">
            {/* Ambient Background Elements */}
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[50%] bg-[#85A39E]/5 rounded-full blur-[150px]" />
                ...
                <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full">
                    ...
                    >
                        <div className="space-y-4">
                            ...
                            >
                                <Sparkles className="w-3 h-3 text-[#85A39E]" />
                            ...
                            <CardContent className="p-8 sm:p-12">
                                <form onSubmit={handleSubmit} className="space-y-8">
                                    <div className="space-y-3 text-left">
                                        <Label htmlFor="trackingId" className="text-[10px] uppercase tracking-[0.2em] text-[#85A39E] font-bold ml-1">
                                            {isGeneric ? "Order Reference" : config.orderLabel}
                                        </Label>
                                        <div className="relative group">
                                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/40 group-focus-within:text-[#85A39E]/40 transition-colors" strokeWidth={1.5} />
                                            <Input
                                                id="trackingId"
                                                value={trackingId}
                                                onChange={(e) => setTrackingId(e.target.value)}
                                                placeholder="e.g. KT-7492-X"
                                                className="bg-white/10 border-white/30 h-16 pl-12 rounded-[1.25rem] focus:border-[#85A39E] focus:ring-1 focus:ring-[#85A39E]/20 placeholder:text-white/40 text-lg font-light tracking-widest uppercase transition-all"
                                                required
                                            />
                                        </div>
                                    </div>
                                    <Button
                                        type="submit"
                                        className="w-full bg-[#85A39E] hover:bg-[#85A39E]/90 text-white h-12 rounded-full font-light tracking-[0.1em] text-xs uppercase shadow-xl transition-all active:scale-95 group border-none"
                                    >
                                        Track Order
